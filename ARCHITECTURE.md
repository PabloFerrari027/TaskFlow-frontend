...
(dashboard)/                               Layout protegido — RequireAuth + CurrentWorkspaceProvider + SyncProvider
  ...
  /workspaces/[workspaceId]                Detalhe: membros / convites / atividade / assistente / automações (tabs; a de automações só para OWNER/ADMIN)
...
| **automations** | `/workspaces/:id/automation-rules` (GET/POST/PATCH/DELETE) | Regras "quando X, então Y" que rodam sem confirmação; toda a API é `OWNER`/`ADMIN` (`canManageAutomations`), então a aba "Automações" do workspace nem aparece para os demais. Ver [subseção dedicada](#automações) abaixo. |
