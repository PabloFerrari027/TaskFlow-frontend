...
export default function WorkspaceInvitePage() {
  ...
  const previewQuery = useWorkspaceInvitationPreviewQuery(token);
  ...
  const next = `/invite/workspace/${token}`;

  return (
    <InvitationPreviewCard
      entityLabel="workspace"
      entityName={preview.workspaceName}
      email={preview.email}
      role={preview.role}
      ...
      onAccept={() =>
        acceptMutation.mutate(token, {
          onSuccess: () => {
            toast.success(`Você agora faz parte de ${preview.workspaceName}.`);
